custom_imports = dict(
    imports=["elastic_vit_od.models", "elastic_vit_od.engine"],
    allow_failed_imports=False,
)

# -----------------------------------------------------------------------------
# Set this to the Elastic-ViT stage-2 checkpoint before training.
backbone_checkpoint = None  # e.g. "../Elastic-ViT/outputs/elastic/imagenet1k/elastic_epoch_30.pt"
...
# CocoMetric reports AP, AP50, AP75 (plus AP_s/m/l) for each requested task.
val_evaluator = dict(
    type="CocoMetric",
...
# Lower LR for the (pretrained) ViT backbone, full LR for the freshly-init heads.
optim_wrapper = dict(
    type="OptimWrapper",
...
# Elastic weight-shared training: sample one sub-network per iteration.
custom_hooks = [
    dict(
        type="ElasticConfigHook",
        mlp_choices=mlp_choices,
        head_choices=head_choices,
