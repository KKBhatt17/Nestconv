...
from .adaptive_patching import AdaptivePatchSequenceBuilder
...
class EntropyConstraintPredictor(nn.Module):
    def __init__(self, hidden_dim=16, min_constraint=0.2, max_constraint=1.0):
        super().__init__()
    ...
    def forward(self, entropy_mean):
        if entropy_mean.dim() == 0:
            entropy_mean = entropy_mean.view(1, 1)
        elif entropy_mean.dim() == 1:
            entropy_mean = entropy_mean.unsqueeze(-1)

        normalized_constraint = torch.sigmoid(self.network(entropy_mean))
        constraint = self.min_constraint + (self.max_constraint - self.min_constraint) * normalized_constraint
        return constraint.squeeze(-1)
...
class EAViTStage2(timm.models.vision_transformer.VisionTransformer):
    def __init__(
            ...
            constraint_min=0.2,
            constraint_max=1.0,
            adaptive_num_scales=2,
            adaptive_thresholds=(5.5,),
            **kwargs,
    ):
        ...
        self.depth = depth
        self.embed_dim = embed_dim
        patch_size = self.patch_embed.patch_size[0] if isinstance(self.patch_embed.patch_size, tuple) else self.patch_embed.patch_size
        image_size = self.patch_embed.img_size[0] if isinstance(self.patch_embed.img_size, tuple) else self.patch_embed.img_size
        ...
        self.constraint_predictor = EntropyConstraintPredictor(
            hidden_dim=predictor_hidden_dim,
            min_constraint=constraint_min,
            max_constraint=constraint_max,
        )
        self.adaptive_patching = AdaptivePatchSequenceBuilder(
            image_size=image_size,
            patch_size=patch_size,
            embed_dim=embed_dim,
            num_scales=adaptive_num_scales,
            thresholds=adaptive_thresholds,
        )
        self.last_token_count = torch.tensor(float(self.patch_embed.num_patches))

    def _forward_sequence(self, x: torch.Tensor):
        if self.training:
    ...
    def caculate_cost(self):
        ...
        N = self.last_token_count.clamp_min(1.0)
    ...
    def predict_constraint(self, entropy_mean):
        return self.constraint_predictor(entropy_mean)
