...
def routing_to_subnetwork_config(routing, router, sample_idx: int = 0) -> SubnetworkConfig:
    ...
    return SubnetworkConfig(
        mlp_widths=tuple(router.mlp_choices[index.item()] for index in mlp_indices),
        num_heads=tuple(router.head_choices[index.item()] for index in head_indices),
    )
...
def route_single_image(
    ...
    config = routing_to_subnetwork_config(routing, router)
    if export_subnetwork:
        runtime_model = model.export_subnetwork(config).to(device)
        logits = runtime_model(tensor)
