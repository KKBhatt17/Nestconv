"""Evaluate an Elastic-ViT segmentor at N fixed backbone sub-networks.

For each sub-network listed in ``--configs-file`` this reports the mIoU (and the
other IoUMetric outputs: aAcc/mAcc) together with the analytic backbone GMACs,
then writes a summary table to the work dir.
...
from elastic_vit_ss.models.common import make_subnetwork_config  # noqa: E402
from elastic_vit_ss.utils.flops import estimate_backbone_gmacs  # noqa: E402


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate Elastic-ViT SS over N backbone configs")
    ...
    parser.add_argument("--input-hw", type=int, nargs=2, default=(512, 512), metavar=("H", "W"), help="Representative eval input size for the GMACs estimate")
