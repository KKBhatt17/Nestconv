...
def get_black_image(image: PIL.Image.Image) -> PIL.Image.Image:
    return PIL.Image.new('RGB', image.size, (0, 0, 0))
...
class FashionIQDataset(Dataset):
    ...
    def __init__(self, dataset_path: Union[Path, str], split: Literal['train', 'val', 'test'], dress_types: List[str],
                 ...
                 blip_transform: callable = None, black_image: bool = False):
        """
        ...
        try:
            if self.mode == 'relative':
                ...
                if self.split in ['train', 'val']:
                    ...
                    reference_pil = PIL.Image.open(reference_image_path).convert('RGB')
                    if self.black_image:
                        reference_pil = get_black_image(reference_pil)
                    reference_image = self.preprocess(reference_pil)
                    target_name = self.triplets[index]['target']
                    target_image_path = self.dataset_path / 'images' / f"{target_name}.png"
                    target_pil = PIL.Image.open(target_image_path).convert('RGB')
                    target_image = self.preprocess(target_pil)
                    blip_ref_img = self.blip_transform(reference_pil)
                    blip_target_img = self.blip_transform(target_pil)
