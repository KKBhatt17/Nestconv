...
def apply_empty_caption_ablation(args: argparse.Namespace, captions: List[str]) -> List[str]:
    if getattr(args, 'empty_caption', False):
...
def generate_predictions(
    ...
    relative_captions = apply_empty_caption_ablation(args, relative_captions)
    ...
    if preload_dict['mods'] is None or not os.path.exists(preload_dict['mods']):
        ...
        for i in tqdm.trange(len(all_captions), position=1, desc=f'Modifying captions with LLM...', leave=False):
            ...
            resp = llm_api.ollama_completion(
                final_prompt,
