...
def sanitize_cache_tag(value: str) -> str:
...
def main():
    ...
    parser.add_argument("--openai_engine", default='gpt-3.5-turbo', type=str, choices=['gpt-3.5-turbo', 'gpt-4'],
                        help='Deprecated legacy argument. It is ignored because caption modification uses Ollama now.')
    ...
    parser.add_argument("--black_image", action='store_true',
                        help='Use a complete black reference image for query-time image captioning ablations.')
    parser.add_argument("--empty_caption", action='store_true',
                        help='Use an empty relative caption string for query-time text ablations.')
    parser.add_argument("--llm_model", type=str, default='llama3.1:8b',
                        help='Ollama model name used to modify captions.')
    ...
    parser.add_argument("--llm_temperature", type=float, default=0.0,
                        help='Sampling temperature for the Ollama LLM call.')
    ...
    caption_ablation_suffix = '_blackref' if args.black_image else ''
    mod_ablation_parts = []
    if args.black_image:
        mod_ablation_parts.append('blackref')
    if args.empty_caption:
        mod_ablation_parts.append('emptycap')
    mod_ablation_suffix = '' if not mod_ablation_parts else '_' + '_'.join(mod_ablation_parts)
    llm_model_tag = sanitize_cache_tag(args.llm_model)
    ...
    if 'captions' in args.preload:
        ...
        if args.blip_prompt != 'prompts.blip_prompt':
            preload_dict['captions'] = os.path.join(
                'precomputed',
                caption_load_str + f'_captions_{args.blip_prompt.split(".")[-1]}{caption_ablation_suffix}.pkl'
            )
        else:
            preload_dict['captions'] = os.path.join(
                'precomputed',
                caption_load_str + f'_captions{caption_ablation_suffix}.pkl'
            )
            
    if 'mods' in args.preload:
        # # LLM-based caption modifications depend on prompts, ablations, and the chosen local LLM.
        mod_load_str = f'{args.dataset}_{args.blip}_{args.split}'.replace('/', '-')
        preload_dict['mods'] = os.path.join(
            'precomputed',
            mod_load_str + f'_mods_{args.llm_prompt.split(".")[-1]}_{llm_model_tag}{mod_ablation_suffix}.json'
        )
    
    if args.split == 'test':
        test_suffix = mod_ablation_suffix + f'_{llm_model_tag}'
        preload_dict['test'] = (
            preload_str + f'_{args.blip_prompt.split(".")[-1]}_{args.llm_prompt.split(".")[-1]}'
            + f'{test_suffix}_test_submission.json'
        )
    ...
    if 'fashioniq' in args.dataset.lower():
        ...
        query_datasets.append(
            datasets.FashionIQDataset(
                args.dataset_path, args.split, [dress_type], 'relative', preprocess,
                blip_transform=vis_processors["eval"], black_image=args.black_image
            )
        )
        ...
        query_datasets.append(
            datasets.CIRRDataset(
                args.dataset_path, split, 'relative', preprocess,
                blip_transform=vis_processors["eval"], black_image=args.black_image
            )
        )
